...
module.exports = async (req, res) => {
  try {
    ...
    // =============================
    // TESTE RÁPIDO DE DEPLOY
    // =============================
    if (userText === "VERCEL_TESTE_01") {
      await sendWhatsAppText(from, "✅ TESTE OK - webhook novo em produção");
      return res.status(200).json({ ok: true, route: "vercel-test" });
    }
    ...
    if (isAddressRequest(userText)) {
      ...
      return res.status(200).json({ ok: true, route: "address-direct-top" });
    }
    ...
    // =============================
    // IMAGEM
    // OCR REMOVIDO PARA NÃO QUEBRAR VERCEL
    // =============================
    ...
    if (isAddressRequest(userText)) {
      ...
      return res.status(200).json({ ok: true, route: "address-direct-after-media" });
    }
